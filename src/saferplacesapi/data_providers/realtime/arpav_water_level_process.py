...
class ARPAVWaterLevelProcessor(BaseProcessor):
    ...
    def create_geojson_dataset(self, gdf):
        gdf_agg = gdf.groupby(by='codice_stazione').aggregate({ prop: 'first' for prop in self.properties if prop in gdf.columns } | { 'valore': list, 'date_time': list }).reset_index(drop=True)
        ...
        geojson_dataset_filename = _processes_utils.get_data_filename(dataset_name = self.dataset_name, variable_name = self.variable_name, time_range = gdf['date_time'].max(), extension='geojson')
        geojson_dataset_filepath = os.path.join(self._data_folder, geojson_dataset_filename)
